...
class LectureMyLikeActivity :
    BaseActivity<ActivityLectureMylikeBinding>(R.layout.activity_lecture_mylike) {

    private val viewModel: LectureMyLikeViewModel by viewModel()
    private lateinit var adapter: LectureMyLikeAdapter
    ...
    private fun initViewModelObserve() {
        with(viewModel) {
            _lecture.observe(this@LectureMyLikeActivity , Observer{
                initAdapter(it!!.lectureList)
            })

            networkFail.observe(this@LectureMyLikeActivity, Observer {
                showNetworkFail()
            })
            serverError.observe(this@LectureMyLikeActivity, Observer {
                showToast(ToastType.ERROR, it)
            })
            appRefresh.observe(this@LectureMyLikeActivity, Observer {
                goRefresh()
            })
        }
    }
    ...
    private fun initAdapter(lectList : List<Lecture>) {
        ...
        adapter = LectureMyLikeAdapter(lectureList,LectureItemClick())
